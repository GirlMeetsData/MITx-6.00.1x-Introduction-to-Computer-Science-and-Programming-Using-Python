...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        alphabet_upper = [letter for letter in string.ascii_uppercase]
        alphabet_lower = [letter for letter in string.ascii_lowercase]
        ...
        for i in range(len(alphabet_upper)):
            shiftedDict[alphabet_upper[i]] = shifted_alphabet_upper[i]
    
        for i in range(len(alphabet_lower)):
            shiftedDict[alphabet_lower[i]] = shifted_alphabet_lower[i]
    ...
    def apply_shift(self, shift):
        ...
        for letter in self.message_text:
            if letter not in shifted:
                ans.append(letter)
            else:
                ans.append(shifted[letter])
        return "".join(ans)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        orig_text = self.message_text
        max_count = 0
        best_shift = 0
        for shift_val in range(0, 26):
            local_count = 0
            self.message_text = orig_text
            self.message_text = self.apply_shift(shift_val)
...
#Example test case (Decrypt a Story)
def decrypt_story():
    story = get_story_string()
    cipher = CiphertextMessage(story)
    result = cipher.decrypt_message()
    return result
